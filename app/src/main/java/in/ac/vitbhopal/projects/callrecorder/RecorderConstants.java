package in.ac.vitbhopal.projects.callrecorder;

public class RecorderConstants {
    private RecorderConstants() { }

    public static final String ACTION_BR_PROJECTIONDATA = "ProjectionDataReceiveBroadcast";
    public static final String BR_RESULT_PROJECTIONDATA = "ProjectionResultData";
    public static final String DEBUG_TAG = "JCallRecorder";
    public static final int REQUESTCODE_ACCESSIBILITY = 50;
    public static final int REQUESTCODE_PERMISSIONS = 51;
    public static final int REQUESTCODE_MEDIAPROJECTION = 52;
    public static final String NOTIFICATION_CHANNEL_ID = "JCR";
}
